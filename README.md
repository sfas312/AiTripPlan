# AiTripPlan：多 Agent 协同旅行规划系统

## 项目概述

针对传统单一大语言模型在复杂旅行规划中存在长链任务分解不足、外部工具调用不稳定以及多约束条件难以同时满足的问题，AiTripPlan 基于 **AgentScope** 与 **A2A** 构建多 Agent 协同旅行规划框架。系统将复杂出行需求拆分为路线规划、行程安排、住宿餐饮、预算约束和结果整合等子任务，并通过 Nacos 完成远程 Agent 的注册与发现。

`ManagerAgent` 基于 **ReAct** 完成用户需求解析、任务拆解和子任务调度；`RouteMakingAgent` 与 `TripPlannerAgent` 通过 Skills 分别承担候选景点与 POI 信息组织、路线查询、日程编排和结果整合等工作。路线 Agent 接入**百度地图 MCP** 获取在线路线信息，输出里程、耗时、过路费、道路摘要及交通建议。

系统保留 `PlanNotebook`、Hook 调用链、结构化结果和规则化质量评分，覆盖路线可达性、日程安排、预算与用户约束等关键检查项；地图服务调用异常时会返回明确失败原因，并支持演示环境下的预设路线。项目配套提供单 LLM、无 MCP 多 Agent 与带 MCP 多 Agent 的对比实验脚本，用于分析路线规划、行程规划、预算满足、约束覆盖和响应时延等指标。
